##################################################################################
# Main Script for Training and Testing the MA-LSTM-MLP on Datasets of Dynamical Systems
...
# Check arguments:
if len(sys.argv) != 3:
    print("Use: python ma_lstm_mlp.py <dataset> <train|test>")
    exit()
...
scaler_input, scaler_state = data.load_dataset(config, test)
...
#Test
    ...
    r2, rmse, mae = plot.print_metrics(config, state_test, state_nn)
    plot.plot_loss(config, network.get_model_name())
    plot.plot_all_series(config, input_test, state_test, state_nn, r2, rmse, mae)