...
def load_dataset(config, test):

    print("Loading ["+config.dataset_name+"] dataset ...")
    start_time = time.time()

    #Data splits -> (batch, length, parameters)
    input_data = []
    state_data = []
...
#Scale indput_data and state_data with scalers
...
def preprocess_data(input_data, state_data, config, training):
    ...
    if config.length == -1:
        for batch in range(input_data.shape[0]):
            ...
            if config.buffer == 0:
                input_pre.append(input_batch)
                state_pre.append(state_batch)
            else:
