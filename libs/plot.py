...
def print_metrics_i(config, r2, rmse, nrmse):
    print("NRMSE:")
    for i in range(config.state_size):
...
def plot_all_series(config, input_test, state_test, state_nn, r2, rmse, nrmse):
    n_series = len(state_nn)
    for serie in range(n_series):
        print("Test serie = ",serie)
        print_metrics_i(config, r2[serie], rmse[serie], nrmse[serie])
...
def plot_serie_i(config, input_test, state_test, state_nn):
    ...
    if config.dataset_name == 'wiener_hammerstein':
        plt.figure(1, figsize=(6, 2))
        
        font_size = '8'

        plt.plot(length, state_test[:, 0], color=COLOR_2, label="y")
        ...
        plt.ylabel('$Y$')
        plt.title('Wiener-Hammerstein Swept sine Benchmark')
        
        plt.subplots_adjust(left=0.13, bottom=0.22, right=0.99, top=0.86, hspace=0.20)
        plt.show()

    if config.dataset_name == 'robot':
